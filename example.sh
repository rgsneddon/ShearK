#!/bin/sh
# ShearK-Miner 2.4 (ShearHash-v3 light)
...
if [ ! -x ./ShearK-Miner ]; then
  echo "ShearK-Miner missing or not executable. Unpack ShearK-Miner-2.3-linux.zip first."
  exit 1
fi

exec ./ShearK-Miner --pool pool.shear.digital:1111 --user YOUR_SSA1.worker --backend jit-full --threads 8
